<script lang="ts">
  ...
  let gradientStops = $derived.by(() => {
    ...
    for (let i = 0; i <= 10; i++) {
      ...
      const mockRow = {
        _index: 0,
        [colorScale.column]: val,
      } as import('../../types/data').DesignRow;
      stops.push({
        offset: `${t * 100}%`,
...
{#if colorScale}
  <div class="p-3">
    ...
    <button
      class="mt-2 text-[10px] text-gray-400 hover:text-gray-600 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded"
